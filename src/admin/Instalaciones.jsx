...
import { apiFetch, fmtDate, fmtTime, styles } from './utils.js'
...
export default function InstalacionesSection() {
  ...
  const [submitOk, setSubmitOk]       = useState(null)   // { link }
  ...
  // Cargar cotizaciones confirmadas para el selector
  useEffect(() => {
  ...
  async function handleAgendar(e) {
    ...
    try {
      ...
      if (data.ok) {
        setSubmitOk(data)
        // Reset form
        setSelectedQuote(null); setManualNombre(''); setManualEmail(''); setManualTelefono(''); setManualDireccion('')
  ...
  return (
    <div>
      ...
      <div style={{ ...styles.card, marginBottom: 20 }}>
        <div style={styles.cardLabel}>Agendar nueva instalación</div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {!useManual && (
            confirmedQuotes.length === 0
              ? <p style={{ color: '#aaa', fontSize: 13 }}>No hay cotizaciones confirmadas aún. Confírmalas en la sección Cotizaciones.</p>
          ...
          {useManual && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                ...
                { label: 'Teléfono', val: manualTelefono, set: setManualTelefono, placeholder: '+56 9 XXXX XXXX' },
                { label: 'Dirección', val: manualDireccion, set: setManualDireccion, placeholder: 'Dirección de instalación', full: true },
        ...
        <form onSubmit={handleAgendar}>
          ...
          {submitOk && (
            <div style={{ background: '#d1fae5', border: '1px solid #6ee7b7', borderRadius: 8, padding: '12px 16px', marginBottom: 12, fontSize: 13, color: '#065f46' }}>
              ✅ Instalación agendada en Google Calendar.{' '}
          ...
          <button type="submit" disabled={submitting || (!useManual && !selectedQuote)}
            ...
            {submitting ? '⏳ Agendando…' : '📅 Agendar instalación en Calendar'}
          </button>
        </form>
      </div>
      ...
      {!loadingList && !listError && (
        <>
          ...
          {upcoming.length === 0 && past.length === 0 && (
            <div style={styles.empty}>No hay instalaciones agendadas aún.</div>
          )}
        </>
      )}
    </div>

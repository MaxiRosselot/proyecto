...
// Botones de cambio de estado segun pestana activa
// agendada -> pendiente | cancelada | realizada | reagendar
// pendiente -> cancelada | realizada | reagendar
// realizada -> (solo info, desde aca se cotiza)
// realizada_cotizada -> (solo info)
// cancelada -> (solo info)
...
export default function VisitasSection({ statuses, onStatusChange, navigateTo, onVisitsLoaded }) {
  ...
  async function handleStatus(visit, newStatus) {
    ...
    try {
      ...
      if (newStatus === 'reagendar') {
        // reagendar = vuelve a agendada, cambiamos localmente
        onStatusChange(visit.id, 'agendada')
      }
  ...
  function getStatus(visit) {
    return statuses[visit.id] || 'agendada'
  }

  // Clasificacion de visitas en pestanas
  // Agendadas: fecha futura y estado agendada/reagendar
  // Pendientes: fecha pasada y sin estado declarado (agendada/reagendar)
  // Realizadas: estado realizada (sin cotizar)
  // Realizadas y cotizadas: estado realizada_cotizada
  ...
  const listMap = {
    ...
    pendientes,
    realizadas,
    realizadas_cotizadas: realizadasCotizadas,
    canceladas,
  }
              ...
              <div onClick={() => setExpanded(isExpanded ? null : visit.id)}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 13, color: C.textSub }}>
                    {fmtDate(visit.start)}
                    {fmtTime(visit.start) && <span style={{ marginLeft: 8 }}>{fmtTime(visit.start)}</span>}
